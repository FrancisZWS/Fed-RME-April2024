...
'''may encounter GPU RAM overflow'''
class AutoEncoderEstimator_Cent(BemMapEstimator):
    """
       Arguments:
    ...
    def __init__(self,
                 ...
                 save_as='weights.h5',
                 **kwargs):
        super(AutoEncoderEstimator_Cent, self).__init__(**kwargs)
        self.str_name = "Centralized"
        ...
        self.chosen_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2),  #, global_clipnorm=10000.
...
# Added __getstate__ and __setstate__ to make it serializable (failed)
    def __getstate__(self):
        # Remove any non-serializable attributes or objects
        state = self.__dict__.copy()
        # Example: If there are non-serializable objects like TensorFlow models or callbacks, remove them from state
        del state['chosen_model']
        return state

    def __setstate__(self, state):
        # Restore the object's state during unpickling
        self.__dict__.update(state)
        # Example: If you removed any attributes in __getstate__, recreate them here
        self.chosen_model = None  # Recreate the TensorFlow model during unpickling
    ...
    def train(self,
              ...
              **kwargs):
        # hist = []
        hist = np.zeros((3, n_epochs * n_super_batches))
        latent_vars = []
        nsb_range = range(n_super_batches)
        for ind_s_batch in range(n_super_batches):
            l_rate = learning_rate / (1.2 ** ind_s_batch) #2 ** ind_s_batch
            # l_rate = learning_rate
            history, latent_vars = self.train_one_batch(generator, sampler, l_rate, sp_batch_idx=ind_s_batch, n_epochs=n_epochs, **kwargs)
            print("Superbatch " + str(ind_s_batch) + " out of " + str(nsb_range[-1]) + " complete.")
            print('Learning rate', l_rate)
            gc.collect()
            a = 1

            hist[:, ind_s_batch * n_epochs: ind_s_batch * n_epochs + n_epochs] = history
    ...
    def train_one_batch(self,
        ...
        def ll_to_nparray(str_key):
            return np.array(
                [data_point[str_key] for l_data_points in self.l_l_data_points for data_point in l_data_points])
        ...
        # Generate all data points using parallel processing
            ...
            self.l_l_data_points = Parallel(n_jobs=num_cores, backend='threading')(delayed(process_one_map)(ind_map)
        ...
        # Get arrays from l_l_data_points
            ...
            self.t_x_points = ll_to_nparray("x_point")
            self.t_y_points = ll_to_nparray("y_point")
            self.t_y_masks = ll_to_nparray("y_mask")
            self.t_channel_pows = ll_to_nparray("channel_power_map")

            end_time = time.time()
            elapsed_time = end_time - start_time #shows 01:00:03:01 fpr 1W data, not accurate
        ...
        overall_n_maps = self.t_x_points.shape[0]
        n_maps_train = int(perc_train * overall_n_maps)

        # x_points
        t_x_points_train = self.t_x_points[0:n_maps_train]
        t_x_points_valid = self.t_x_points[n_maps_train:]

        # y_points
        t_y_points_train = self.t_y_points[0:n_maps_train]
        t_y_points_valid = self.t_y_points[n_maps_train:]

        # y_masks
        t_y_masks_train = self.t_y_masks[0:n_maps_train]
        t_y_masks_valid = self.t_y_masks[n_maps_train:]

        # training channel powers
        t_channel_pows_train = self.t_channel_pows[0:n_maps_train]
        ...
        self.chosen_model.optimizer.learning_rate = l_rate
        ...
        #self.chosen_model.save_weights('output/autoencoder_experiments/savedWeights/test.h5')
    ...
    def format_preparation(self,
                           ...
                           m_mask_in,
                           t_output,
                           m_mask_out, #1 - m_meta_map (48x48)
                           ):
        """
        Inputs: t_sampled_map, m_meta_map, m_mask,t_map, m_mask_out
        Returns:
    ...
    #outputs: t_sampled_map_in, v_map_out, v_mask_out
