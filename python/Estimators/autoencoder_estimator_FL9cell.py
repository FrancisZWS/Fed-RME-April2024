# Weishan Version Dec 4. Dataset should be generated before training and remain unchanged for one user.
import time
...
class AutoEncoderEstimator_FL9Cell(BemMapEstimator):
    """FL preliminary test, a map cut into 9 cells, for Romero Wireless Insite data
       Arguments:
    ...
    def __init__(self,
                 ...
                 overlap = 0,
                 **kwargs):
        # super(AutoEncoderEstimatorSSC_FL9Cell, self).__init__(**kwargs)
        super().__init__(**kwargs)
        self.str_name = "FL with Overlap"+str(overlap)
        ...
        self.Nx=3
        self.Ny=self.Nx
        self.overlap = overlap #  overlapping btw adjacent cells
        # self.map_list = scipy.io.loadmat('map_list4w.mat')['Traindata'][0]
        ...
        self.chosen_model = getattr(Autoencoder(height=self.n_grid_points_x+ 2*self.overlap, #run the func called 'architecture_name' in obj 'Autoencoder' to build and return DNN
                                                width=self.n_grid_points_y+ 2*self.overlap,
        ...
        # self.chosen_model.compile(optimizer=tf.compat.v1.train.GradientDescentOptimizer(learning_rate=1e-6),loss='mse',sample_weight_mode='temporal', weighted_metrics=[]) #resolve a warning
        ...
        # self.chosen_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2, clipvalue=10.0),
        self.chosen_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2,   global_clipnorm=100. ),
        ...
        for w_ind in range(self.n_walkers):
            self.chosen_models.append(getattr(Autoencoder(height=self.n_grid_points_x+ 2*self.overlap,
                                                width=self.n_grid_points_y+ 2*self.overlap,
                                                c_len=self.code_length,
                                                add_mask_channel=self.add_mask_channel,
                                                mask_as_tensor=self.use_masks_as_tensor,
            ...
            self.chosen_models[w_ind].compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2, global_clipnorm=1000.),
                                              loss= tf.keras.losses.mean_squared_error, #'mse',
                                              sample_weight_mode='temporal',
                                              weighted_metrics=[])
        ...
        if weight_file: #load central model
            self.chosen_model.load_weights(weight_file)

        if weight_file: #load each node
            delim = '/'
            sp = weight_file.split(delim)
            nname = sp[-1]

            # Load the federated weights to all nodes
            if load_all_weights:
                for w_ind in range(self.n_walkers):
                    if (nname[-3:] == '.h5'): #if given .../weights.h5 as file dir
                        delim = '.'
                        sp = nname.split(delim)
                        fname = sp[0] + '_' + str(w_ind) + delim + sp[1]
                        print(fname)
                        # self.chosen_models[w_ind].load_weights( os.path.dirname(weight_file) +'/'+ fname ) #load weight_n.h5
                        self.chosen_models[w_ind].load_weights(weight_file) #load weight.h5
    ...
    def estimate_map(self, sampled_map, mask, meta_data, DifShap=None, usr = -1):
        """
        :DifShap=(16,16,1): if not None, reshape to it; if None, reshape to sampled_map.
        ...
        if usr > -1:
            reconstructed = self.chosen_models[usr].predict(x=sampled_map_feed)
        else:
            reconstructed = self.chosen_model.predict(x=sampled_map_feed)

        if DifShap:
            out = np.reshape(reconstructed[0, :, :], DifShap)
        else:
            out = np.reshape(reconstructed[0, :, :], sampled_map.shape)

        return out

    # def estimate_map_usr(self, sampled_map, usr_idx, mask, meta_data, DifShap=None):
    #     """
    #     :DifShap=(16,16,1): if not None, reshape to it; if None, reshape to sampled_map.
    #     :param sampled_map:  the sampled map with incomplete entries, 2D array with shape (n_grid_points_x,
    #                                                                                        n_grid_points_y)
    ...
    #     else:
    #         sampled_map_feed = np.expand_dims(sampled_map, axis=0)
    #     reconstructed = self.chosen_models[usr_idx].predict(x=sampled_map_feed)
    #     if DifShap:
    #         out = np.reshape(reconstructed[0, :, :], DifShap)
    #     else:
    #         out = np.reshape(reconstructed[0, :, :], sampled_map.shape)
    ...
    def train_fed(self,
        ...
        for ind_s_batch in range(n_super_batches):
            ...
            l_rate = learning_rate * ((.9)**(ind_s_batch/2) ) # AO: Slower learning rate reduction
            print('=====****=====learning rate:', l_rate)

            if self.overlap:
                history, latent_vars = self.train_one_batch_EGoverlap(generator, sampler, l_rate, n_epochs=n_epochs,
                                                            sp_batch_idx=ind_s_batch,
                                                            **kwargs)  # AO: 'hist' was temporarily 'history'
            else:
    ...
    def train_one_batch(self,
        ...
        def process_a_mat_map(user_idx, ind_map, pt_map = None, pm_meta_map = None, pt_ch_power = None, pt_sampled_map=None, pm_mask=None):
            """
            Weishan: Different version of process_a_mat_map
            for matlab data, for SSC data, for only 1024 true maps
            ind_map: not used
            ...
            if pt_map is None: # if not prepared, generate one; if prepraed, just use it
                # t_map = self.map_list[ind_map].copy()[:,:,np.newaxis] # .reshape((32,32,1))
                Cell_xy = index2Cell(user_idx, self.Nx, self.Ny) # Cell_xy  =(x,y), xy are int
                t_mapfull = random.choice(self.map_list).copy()[:, :, np.newaxis]
                CellWid = self.n_grid_points_x # cell width, 16
                CellLen = self.n_grid_points_y  # cell height, 16
                t_map = t_mapfull[Cell_xy[0]*CellWid: (Cell_xy[0]+1)*CellWid,  Cell_xy[1]*CellLen: (Cell_xy[1]+1)*CellLen]
                # print('============check true map shape',t_map.shape)
                m_meta_map = np.zeros((self.n_grid_points_x, self.n_grid_points_y))
                t_ch_power = t_map.copy()
            ...
            t_sampled_map, m_mask = sampler.sample_map(t_map, m_meta_map)
            if v_split_frac == 1: # When complete map is given for groundtruth/label
            ...
            elif len(v_split_frac) == 2: # When complete map is not given for groundtruth
                l_data_points = []
        ...
        if not sp_batch_idx: #if the first superbatch, need to make data. Save train/valid/mask in lists
            ...
            for w_ind in range(self.n_walkers): #iterate to make data for walkers
                ...
                self.datasets.append( # Generate all data points using parallel processing
                                Parallel(n_jobs=num_cores, backend='threading')(delayed(process_a_mat_map)(w_ind, ind_map, t_map, m_meta_map, t_ch_power, t_sampled_map, m_mask)
...
# =================updated training part to work with lists of train/valid/mask
    ...
    def train_one_batch_EGoverlap(self,
                        generator,
                        sampler,
                        l_rate,
                        n_maps=128,
        ...
        """
        New version with overlaping input edge btw adjacent cells:
        input: 17x17, output: 16x16
        Edge cells use zero padding. No data out of 48x48
        """
        ...
        def process_a_mapFLEG(user_num, ind_map):
            """
            Weishan: FL version, handles local users inside, involve overlapping edge with adjacent cells 16x16 => 20x20
            ind_map: not used
            process a map or resamplings of a map
            with keys "x_point", "y_point", "y_mask", and "channel_power_map"

            """
            ovp = self.overlap

            t_mapfull, m_meta_map, t_ch_power = generator.generate() #48x48x1, 48x48, ?
            t_sampled_map, m_mask = sampler.sample_map(t_mapfull, m_meta_map) #48x48x1, 48x48
            ...
            if v_split_frac == 1:
                ...
                t_mapfull_pd = np.pad(t_mapfull, pad_width=pad_width, mode='constant', constant_values=0)
                m_meta_map_pd = np.pad(m_meta_map, pad_width=pad_width[0:1], mode='constant', constant_values=0)
                m_meta_map_pd1 = np.pad(m_meta_map, pad_width=pad_width[0:1], mode='constant', constant_values=1) #for loss-weight, meta_map & 1 padding
                t_sampled_map_pd = np.pad(t_sampled_map, pad_width=pad_width, mode='constant', constant_values=0)
                m_mask_pd = np.pad(m_mask, pad_width=pad_width[0:1], mode='constant', constant_values=0)

                for usr_idx in range(user_num):
                    Cell_xy = index2Cell(usr_idx, self.Nx, self.Ny)  # Cell_xy  =(x,y), xy are int
                    t_map_FL = t_mapfull_pd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2*ovp, #For local label
                            Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2*ovp]
                    m_meta_map_FL = m_meta_map_pd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2*ovp, # city map
                            Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2*ovp]
                    t_sampled_map_FL = t_sampled_map_pd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2*ovp, #For input
                            Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2*ovp]
                    m_mask_FL = m_mask_pd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2*ovp, #For input sensor location mask
                            Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2*ovp]

                    m_mask_out = 1 - m_meta_map_pd1[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2*ovp, #for loss-weight, meta_map & 1 padding
                            Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2*ovp]

                    t_sampled_map_in_FL, v_map_out_FL, v_mask_out_FL = self.format_preparation(t_sampled_map_FL, m_meta_map_FL, m_mask_FL,
                                                                                      t_map_FL, m_mask_out)
                    self.FL_dataset["X_list"][usr_idx][ind_map] = 0 + t_sampled_map_in_FL
                    self.FL_dataset["Y_list"][usr_idx][ind_map] = 0 + v_map_out_FL
                    # print(ind_map)
                    self.FL_dataset["Mask_list"][usr_idx][ind_map] = 0 + v_mask_out_FL
                ...
                for ind_resample in range(n_resamples_per_map):
                    t_sampled_map_in, m_mask_in, t_sampled_map_out, m_mask_out = sampler.resample_map(
                        t_sampled_map, m_mask, v_split_frac)
                    # Padding:
                    m_mask_outpd = np.pad(m_mask_out, pad_width=pad_width[0:1], mode='constant', constant_values=0)
                    t_sampled_map_outpd = np.pad(t_sampled_map_out, pad_width=pad_width, mode='constant', constant_values=0)
                    m_meta_mappd = np.pad(m_meta_map, pad_width=pad_width[0:1], mode='constant', constant_values=0)
                    t_sampled_map_inpd = np.pad(t_sampled_map_in, pad_width=pad_width, mode='constant', constant_values=0)
                    m_mask_inpd = np.pad(m_mask_in, pad_width=pad_width[0:1], mode='constant', constant_values=0)
                    # Cut for 9 users:
                    for usr_idx in range(user_num):
                        t_sampled_map_inL = t_sampled_map_inpd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2 * ovp,  #
                        Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2 * ovp]
                        m_meta_mapL = m_meta_mappd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2 * ovp,  #
                        Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2 * ovp]
                        m_mask_inL = m_mask_inpd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2 * ovp,  #
                        Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2 * ovp]
                        t_sampled_map_outL = t_sampled_map_outpd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2 * ovp,  #
                        Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2 * ovp]
                        m_mask_outL = m_mask_outpd[Cell_xy[0] * CellWid: (Cell_xy[0] + 1) * CellWid + 2 * ovp,  #
                        Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen + 2 * ovp]

                        t_sampled_map_in_FL, v_map_out_FL, v_mask_out_FL = self.format_preparation(t_sampled_map_inL, m_meta_mapL, m_mask_inL,
                                                                                          t_sampled_map_outL, m_mask_outL)

                        idx = ind_map * n_resamples_per_map + ind_resample
                        self.FL_dataset["X_list"][usr_idx][idx] = 0 + t_sampled_map_in_FL
                        self.FL_dataset["Y_list"][usr_idx][idx] = 0 + v_map_out_FL
                        self.FL_dataset["Mask_list"][usr_idx][idx] = 0 + v_mask_out_FL
        ...
        if not sp_batch_idx:  # if the first superbatch, need to make data. Save train/valid/mask in lists
            ...
            if self.use_masks_as_tensor: # sensor mask in separate dim or not (combine with meta map)
                chn = 3
            else:
                chn = 2

            Lc_sz = self.n_grid_points_x+self.overlap*2 #local map width

            if v_split_frac ==1:
                n_data = n_maps
            elif len(v_split_frac)==2:
                n_data = n_maps*n_resamples_per_map

            self.FL_dataset= {"X_list": [np.empty((n_data, Lc_sz, Lc_sz, chn)) for _ in range(self.n_walkers)], #each array in the list is a input dataset
                              "Y_list": [np.empty((n_data, Lc_sz * Lc_sz, 1)) for __ in range(self.n_walkers)],
                              "Mask_list": [np.empty((n_data, Lc_sz * Lc_sz)) for ___ in range(self.n_walkers)]
                            }

            num_cores = int(multiprocessing.cpu_count())
            Parallel(n_jobs=num_cores, backend='threading')(
                delayed(process_a_mapFLEG)(self.n_walkers, ind_map) for ind_map in range(int(n_maps))
                                                            )

            for w_ind in range(self.n_walkers):  # iterate to make validation tool
                overall_n_maps = self.FL_dataset["X_list"][0].shape[0] # this 'overall' corresponds to current walker
                self.n_train = int(perc_train * overall_n_maps)
                self.internal_eval_list.append(InternalEvaluation(
                    validation_data=(
                    self.FL_dataset["X_list"][w_ind][0:self.n_train],
                    self.FL_dataset["Y_list"][w_ind][0:self.n_train],
                    self.FL_dataset["Mask_list"][w_ind][0:self.n_train])))


            end_time = time.time()
            elapsed_time = end_time - start_time
            print(
                'The elapsed time for generating and sampling training and validation maps using multiple processors '
                'with %d cores is' % num_cores, time.strftime("%H hours %M min %S sec", time.gmtime(elapsed_time)))
            # =================updated training part to work with lists of train/valid/mask
        nw_range = range(self.n_walkers)
        for w_ind in range(self.n_walkers):
            ...
            train_history = self.chosen_models[w_ind].fit(x=self.FL_dataset["X_list"][w_ind][0:self.n_train],
                                                          y=self.FL_dataset["Y_list"][w_ind][0:self.n_train],
                                                          batch_size=batch_size,  # ,
                                                          sample_weight=self.FL_dataset["Mask_list"][w_ind][0:self.n_train],
                                                          # epochs=sp_batch_idx+1, #
                                                          epochs=n_epochs,  # original version
                                                          # initial_epoch = sp_batch_idx, # not exist in original version
                                                          validation_data=(
                                                              self.FL_dataset["X_list"][w_ind][self.n_train:],
                                                              self.FL_dataset["Y_list"][w_ind][self.n_train:],
                                                              self.FL_dataset["Mask_list"][w_ind][self.n_train:],
                                                              ),
                                                          # validation_data=self.valset_list[w_ind],
                                                          callbacks=[self.internal_eval_list[w_ind]],
                                                                     # tensorboard_callback],
            ...
            if (self.save_as[-3:] == '.h5'):
                ...
                self.chosen_models[w_ind].save_weights(
                    'output/autoencoder_experiments/savedWeights/' + self.save_as)
            ...
            # trained_decoder.summary()
            out_layer_ind = len(trained_encoder.layers) - 1
            num_codes_to_show = min(2 * self.n_train, int(3e3))
            codes = get_layer_activations(trained_encoder, self.FL_dataset["X_list"][w_ind][0: num_codes_to_show],
                                          out_layer_ind)

            # # save some l_training_coefficients for later checking
            # n_saved_maps = 500
            # with open('output/autoencoder_experiments/savedResults/True_and_Est_training_bcoeffs.pickle',
            #           'wb') as f_bcoeff:
            #     pickle.dump([self.t_channel_pows_list[w_ind][0:n_saved_maps],
            #                  self.t_x_points_train_list[w_ind][0:n_saved_maps]], f_bcoeff)
    ...
    def format_preparation(self,
        ...
        # t_mask_out = np.repeat(m_mask_out[ovp:-ovp,ovp:-ovp, np.newaxis], t_output.shape[2], axis=2) #cut overlap edge
        t_mask_out = np.repeat(m_mask_out[:, : , np.newaxis], t_output.shape[2], axis=2)
        # print("Shape of m_mask_out:", m_mask_out.shape)
        # print("Type of m_mask_out:", type(m_mask_out))
