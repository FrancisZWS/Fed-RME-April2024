...
class AutoEncoderEstimatorSSC_FL9Cell(BemMapEstimator):
    ...
    def __init__(self,
        ...
        if isinstance(map_dir, list) and all(isinstance(item, str) for item in map_dir):
            map_list=[]
            print('Multiple freqs in training')
            for ind_map in map_dir:
                # map_list = scipy.io.loadmat(ind_map)['TrueMap'][0]
                # print(type(scipy.io.loadmat(ind_map)['TrueMap'][0]))
                map_list.append(scipy.io.loadmat(ind_map)['TrueMap'][0])
            self.map_list = np.concatenate(map_list)
        elif isinstance(map_dir, str): # got map tensors in ndarray
            print('Single freq in training')
            self.map_list = scipy.io.loadmat(map_dir)['TrueMap'][0]
        else:
            print('invalid true map directory')
        print('Read map format:+++++++++++++', type(self.map_list), self.map_list.shape)
        # self.map_list = scipy.io.loadmat(map_dir)['TrueMap'][0]
    ...
    def estimate_map(self, sampled_map, mask, meta_data, DifShap=None):
        """
...
# =================updated training part to work with lists of train/valid/mask
    ...
    def train_one_batch_EGoverlap(self,
        ...
        # These should be lists, as opposed to numpy arrays.
        # function to generate training points
        def process_a_mat_mapEG(user_idx, ind_map, pt_map=None, pm_meta_map=None, pt_ch_power=None,
                              pt_sampled_map=None, pm_mask=None):
            """
            ...
            if pt_map is None:  # if not prepared, generate one; if prepraed, just use it
                ...
                CellWed = self.n_grid_points_x  # cell width, 16
                CellLen = self.n_grid_points_y  # cell height, 16
                t_map__ = t_mapfull[Cell_xy[0] * CellWed: (Cell_xy[0] + 1) * CellWed,
            ...
            if v_split_frac == 1:  # When complete map is given for groundtruth/label
                ...
                t_sampled_map_in, v_map_out, v_mask_out = self.format_preparation(t_sampled_map, m_meta_map, m_mask_core,
                ...
                for ind_resample in range(n_resamples_per_map):
                    ...
                    # reshaping and adding masks
                    t_sampled_map_in, v_map_out, v_mask_out = self.format_preparation(t_sampled_map_in, m_meta_map,
                                                                                      m_mask_in,
                                                                                      t_sampled_map_out, m_mask_out, self.overlap)
            ...
            for w_ind in range(self.n_walkers):  # iterate to make data for walkers
                ...
                self.datasets.append(  # Generate all data points using parallel processing
                    Parallel(n_jobs=num_cores, backend='threading')(
                        delayed(process_a_mat_mapEG)(w_ind, ind_map, t_map, m_meta_map, t_ch_power, t_sampled_map,
                                                   m_mask)
                        for ind_map in range(int(n_maps)))
                )
                # from 'datasets', generate training/valid/mask datasets and save in lists
