...
class AutoEncoderEstimatorSSCFreqs(BemMapEstimator):
    """
       Arguments:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(AutoEncoderEstimatorSSCFreqs, self).__init__(**kwargs)
    ...
    def train_one_batch(self,
        ...
        def process_a_mat_map(ind_map, pt_map = None, pm_meta_map = None, pt_ch_power = None, pt_sampled_map=None, pm_mask=None):
            """
            ...
            if pt_map is None: # if not prepared, generate one; if prepraed, just use it
                ...
                t_map = random.choice(self.map_list).copy()[:, :, np.newaxis]
                ...
                #under single_freq, sizes: (32,32,1), (32,32), (32,32,1)
                # print('########### check generated map sizes ', type(t_map), t_map.shape,
                #       type(m_meta_map),m_meta_map.shape,
                #       type(t_ch_power),t_ch_power.shape )

                # print('=-=-=-=-=-=check array values', '\n',
                #       't_map:',t_map[0], '\n',
                #       'm_meta_map:',m_meta_map[0],'\n',
                #       '****Channel Power:',t_ch_power[0],'\n',
                #       'difference btw t_map and t_ch_power:',np.linalg.norm(t_map - t_ch_power),'\n',
                #       )
                # exit()
            else:
                t_map = pt_map
        ...
        if not sp_batch_idx: #if the first superbatch, need to make data. Save train/valid/mask in lists
            ...
            for w_ind in range(self.n_walkers): #iterate to make data for walkers
                ...
                self.datasets.append( # Generate all data points using parallel processing
                                Parallel(n_jobs=num_cores, backend='threading')(delayed(process_a_mat_map)(ind_map,t_map, m_meta_map, t_ch_power, t_sampled_map, m_mask)
