...
class AutoEncoderEstimatorFed(BemMapEstimator):
    ...
    def __init__(self,
        ...
        self.chosen_model.compile(optimizer=tf.compat.v1.train.AdamOptimizer(),loss='mse',sample_weight_mode='temporal')
        #self.chosen_model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=1e-6, momentum=0.0),loss='mse',sample_weight_mode='temporal')
        #self.chosen_model.compile(optimizer=tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.01),loss='mse', sample_weight_mode='temporal')
        ...
        for w_ind in range(self.n_walkers):
            self.chosen_models.append(getattr(Autoencoder(height=self.n_grid_points_x,
                                                ...
                                                activ_function_name=self.activ_func_name,
                                                est_separately_accr_freq=self.est_separately_accr_freq), architecture_name)())
            self.chosen_models[w_ind].compile(optimizer=tf.compat.v1.train.AdamOptimizer(), loss='mse', sample_weight_mode='temporal')
            #self.chosen_models[w_ind].compile(optimizer=tf.keras.optimizers.SGD(learning_rate=1e-6, momentum=0.0),loss='mse', sample_weight_mode='temporal')
            #self.chosen_models[w_ind].compile(optimizer=tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.01),loss='mse', sample_weight_mode='temporal')
    ...
    def train_fed(self,
        ...
        for ind_s_batch in range(n_super_batches):
            ...
            '''
            # Check that the weights of the first two models, if available, aren't the same.
            if self.n_walkers > 1:
                wc_0 = self.chosen_models[0].weights[0][0, 0, 0, 0].numpy()
                wc_1 = self.chosen_models[1].weights[0][0, 0, 0, 0].numpy()
                if(wc_0 == wc_1):
                    print('FIRST WEIGHTS OF THE FIRST TWO MODELS MATCH; THERE MAY BE AN ERROR!!!')
            '''
            ...
            for w_ind in range(self.n_walkers):
                ...
                if(w_ind == 0):
                    ...
                    for ee in range(len(self.chosen_model.trainable_weights)):
                        self.chosen_model.trainable_weights[ee].assign(self.chosen_models[w_ind].trainable_weights[ee].numpy())
                        self.chosen_model.trainable_variables[ee].assign(self.chosen_models[w_ind].trainable_variables[ee].numpy())
                else:
                    ...
                    for ee in range(len(self.chosen_model.trainable_weights)):
                        ...
                        self.chosen_model.trainable_variables[ee].assign(self.chosen_model.trainable_variables[ee].numpy() + self.chosen_models[w_ind].trainable_variables[ee].numpy())

                # Average the model weights, layer by layer
                for ee in range(len(self.chosen_model.trainable_weights)):
                    self.chosen_model.trainable_weights[ee].assign(self.chosen_model.trainable_weights[ee].numpy() / self.n_walkers)
                    self.chosen_model.trainable_variables[ee].assign(self.chosen_model.trainable_variables[ee].numpy() / self.n_walkers)
                '''
                if (w_ind == 0):
                    # Take "self.chosen_model" and set weights equal to self.chosen_models[0], layer by layer
                    for ee in range(len(self.chosen_model.layers)):
                        #self.chosen_model.trainable_weights[ee].assign(self.chosen_models[w_ind].trainable_weights[ee].numpy())
                        self.chosen_model.layers[ee].set_weights(self.chosen_models[w_ind].layers[ee].get_weights())
                else:
                    # Add the weights from this model to "self.chosen_model", layer by layer
                    for ee in range(len(self.chosen_model.layers)):
                        #self.chosen_model.trainable_weights[ee].assign(self.chosen_model.trainable_weights[ee].numpy() + self.chosen_models[w_ind].trainable_weights[ee].numpy())
                        ...
                        z1 = self.chosen_model.layers[ee].get_weights()
                        z2 = self.chosen_models[w_ind].layers[ee].get_weights()
            ...
            # Average the model weights, layer by layer
            for ee in range(len(self.chosen_model.layers)):
                cmod = self.chosen_model.layers[ee].get_weights()
            ...
            for w_ind in range(self.n_walkers):
                '''
                for ee in range(len(self.chosen_model.trainable_weights)):
                    self.chosen_models[w_ind].trainable_weights[ee].assign(self.chosen_model.trainable_weights[ee].numpy())
                    self.chosen_models[w_ind].trainable_variables[ee].assign(self.chosen_model.trainable_variables[ee].numpy())
                '''
                for ee in range(len(self.chosen_model.layers)):
                    self.chosen_models[w_ind].layers[ee].set_weights(self.chosen_model.layers[ee].get_weights())
    ...
    def train_one_batch(self,
        ...
        for w_ind in range(self.n_walkers):
            ...
            # Save the weights of the federated model
