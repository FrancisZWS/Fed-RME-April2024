...
# Modified by mistake, April 16 2024
import time
...
class AutoEncoderEstimatorRom_FL9Cell(BemMapEstimator):
    """FL preliminary test, a map cut into 9 cells, centralized version
       Arguments:
    ...
    def __init__(self,
        ...
        for w_ind in range(self.n_walkers):
            ...
            self.chosen_models[w_ind].compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2, global_clipnorm=100.),
                                              loss= tf.keras.losses.mean_squared_error, #'mse',
                                              sample_weight_mode='temporal',
                                              weighted_metrics=[])
    ...
    def estimate_map(self, sampled_map, mask, meta_data, DifShap=False):
        """
    ...
    def train_fed(self,
        ...
        for ind_s_batch in range(n_super_batches):
            ...
            l_rate = learning_rate * ((.9)**(ind_s_batch/20) ) # AO: Slower learning rate reduction
            print('=====****=====learning rate:', l_rate)

            if True: #self.overlap:
...
#                 CellWed = self.n_grid_points_x # cell width, 16
#                 CellLen = self.n_grid_points_y  # cell height, 16
#                 t_map = t_mapfull[Cell_xy[0]*CellWed: (Cell_xy[0]+1)*CellWed,  Cell_xy[1]*CellLen: (Cell_xy[1]+1)*CellLen]
...
#         return hist, codes
    ...
    def train_one_batch_EGoverlap(self,
        ...
        # These should be lists, as opposed to numpy arrays.
        # function to generate training points
        def process_a_mat_mapEG(user_idx, ind_map):
            """
            Weishan: process_a_mat_map, involve overlapping edge with adjacent cells 16x16 => 17x17
            ind_map: not used
            Retuns a list of data points  where one data point is a dictionary
            with keys "x_point", "y_point", "y_mask", and "channel_power_map"
            ...
            Cell_xy = index2Cell(user_idx, self.Nx, self.Ny)  # Cell_xy  =(x,y), xy are int
            # t_mapfull = random.choice(self.map_list).copy()[:, :, np.newaxis]
            t_mapfull, m_meta_map, t_ch_power = generator.generate()
            CellWed = self.n_grid_points_x  # cell width, 16
            CellLen = self.n_grid_points_y  # cell height, 16
            t_map__ = t_mapfull[Cell_xy[0] * CellWed: (Cell_xy[0] + 1) * CellWed, #for labels
                    Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen]
            # t_map.shape
            # np.zeros_like(t_mapfull)
            # x_ext = self.Nx * self.n_grid_points_x + 2 * self.overlap
            # y_ext = self.Ny * self.n_grid_points_y + 2 * self.overlap
            # zero_map = np.zeros(x_ext, y_ext,1)

            pad_width = ((ovp, ovp), (ovp, ovp), (0, 0)) # (top, bottom), (left, right), (no padding for the 3rd dimension)
            t_map_padded = np.pad(t_mapfull, pad_width=pad_width, mode='constant', constant_values=0)
            meta_map_padded = np.pad(m_meta_map, pad_width=pad_width[0:1], mode='constant', constant_values=0)

            t_map = t_map_padded[Cell_xy[0] * CellWed: (Cell_xy[0] + 1) * CellWed+2*ovp,
                    Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen+2*ovp]
            meta_map = meta_map_padded[Cell_xy[0] * CellWed: (Cell_xy[0] + 1) * CellWed+2*ovp,
                    Cell_xy[1] * CellLen: (Cell_xy[1] + 1) * CellLen+2*ovp]
            # print('============check true map shape',t_map.shape)
            # m_meta_map = np.zeros((CellWed+2*ovp, CellLen+2*ovp))
            # m_meta_map = np.zeros((CellWed, CellLen))
            # t_ch_power = t_map.copy()
            ...
            # if pt_sampled_map is None:
            t_sampled_map, m_mask = sampler.sample_map(t_map, meta_map)
            # zero the col and row for cells not in the center
            if Cell_xy[0] ==0:
                t_sampled_map[0:ovp,:,:] = 0
                m_mask[0:ovp,:]  =0
            if Cell_xy[0] == self.Nx-1:
                t_sampled_map[-1*ovp: ,:,:] =0
                m_mask[-1*ovp: ,:] =0
            if Cell_xy[1] ==0:
                t_sampled_map[:, 0:ovp,:] =0
                m_mask[:, 0:ovp] =0
            if Cell_xy[1] ==self.Nx -1:
                t_sampled_map[:, -ovp: ,:] =0
                m_mask[:, -ovp: ] =0
            # else:
            #     t_sampled_map = pt_sampled_map
            ...
            if v_split_frac == 1:  # When complete map is given for groundtruth/label
                ...
                # m_mask_and_meta = m_mask - m_meta_map
                m_mask_out = 1 - meta_map

                # reshaping and adding masks
                m_mask_core=m_mask #m_mask[ovp:-ovp,ovp:-ovp]
                t_sampled_map_in, v_map_out, v_mask_out = self.format_preparation(t_sampled_map, meta_map, m_mask_core,
                                                                                  t_map__, m_mask_out, self.overlap)
                ...
                for ind_resample in range(n_resamples_per_map):
                    ...
                    # reshaping and adding masks
                    t_sampled_map_in, v_map_out, v_mask_out = self.format_preparation(t_sampled_map_in, meta_map,
                                                                                      m_mask_in,
                                                                                      t_map__, m_mask_out, self.overlap)
                ...
                l_data_points = None
            # if ind_map%500 == 0:
            #     print('The',ind_map,'-th input sample has been generated')
            return l_data_points
        ...
        if not sp_batch_idx:  # if the first superbatch, need to make data. Save train/valid/mask in lists
            ...
            num_cores = int(multiprocessing.cpu_count())
            self.True_maps = Parallel(n_jobs=num_cores, backend='threading')(
                delayed(generator.generate)() for _ in range(n_maps))
            sampled_a = Parallel(n_jobs=num_cores, backend='threading')(delayed(sampler.sample_map)(self.True_maps[nn][0], self.True_maps[nn][1]) for nn in range(int(n_runs)) )
            ...
            for w_ind in range(self.n_walkers):  # iterate to make data for walkers
                ...
                self.datasets.append(  # Generate all data points using parallel processing
                    Parallel(n_jobs=num_cores, backend='threading')(
                        delayed(process_a_mat_mapEG)(w_ind, ind_map)
                        for ind_map in range(int(n_maps)))
                )
                t_para = time.time()
                print(w_ind, 'User data generation takes:', t_para - start_time)
                # from 'datasets', generate training/valid/mask datasets and save in lists
                t_x_points_list.append(ll_to_nparray("x_point"))
                t_x = time.time()
                print('Partition input dataset:', t_x - t_para)
                t_y_points_list.append(ll_to_nparray("y_point"))
                t_y = time.time()
                print('Partition label dataset:', t_y - t_x)
                t_y_masks_list.append(ll_to_nparray("y_mask"))
                t_msk = time.time()
                print('Partition Y_mask dataset:', t_msk - t_y)
                self.t_channel_pows_list.append(ll_to_nparray("channel_power_map"))
                ...
                self.internal_eval_list.append(InternalEvaluation(
                    validation_data=(
                    self.t_x_points_train_list[-1], t_y_points_train_list[-1], t_y_masks_train_list[-1])))
        ...
        nw_range = range(self.n_walkers)
        for w_ind in range(self.n_walkers):
            print("Training user " + str(w_ind) + " out of " + str(nw_range[-1]) + " now.")
            log_dir = "logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
            ...
            out_layer_ind = len(trained_encoder.layers) - 1
            num_codes_to_show = min(2 * self.n_maps_train, int(3e3))
            codes = get_layer_activations(trained_encoder, self.t_x_points_train_list[w_ind][0: num_codes_to_show],
                                          out_layer_ind)
