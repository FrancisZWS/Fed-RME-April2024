...
class AutoEncoderEstimator_Stdaln9Cell(BemMapEstimator):
    ...
    def __init__(self,
        ...
        self.str_name = "Standalone with Overlap"+str(overlap)
        ...
        if weight_file:
            ...
            # Load local weights to corresponding nodes
            if load_all_weights:
                for w_ind in range(self.n_walkers):
                    if (nname[-3:] == '.h5'):
                        delim = '.'
                        ...
                        # print(fname)
                        # self.chosen_models[w_ind].load_weights('output/autoencoder_experiments/savedWeights/' + fname) #load weight_n.h5
                        # self.chosen_models[w_ind].load_weights(weight_file) #load weight.h5
                        paths = os.path.dirname(weight_file) + '/' + fname
                        print('Load local model from:', paths)
    ...
    def estimate_map(self, sampled_map, usr_idx, mask, meta_data, DifShap=None):
        """
