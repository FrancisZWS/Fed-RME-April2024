...
class AutoEncoderEstimatorFed(BemMapEstimator):
    ...
    def train_fed(self,
        ...
        for ind_s_batch in range(n_super_batches):
            ...
            l_rate = learning_rate * (0.9 ** ind_s_batch) # AO: Slower learning rate reduction
            #l_rate = learning_rate
            history, latent_vars = self.train_one_batch(generator, sampler, l_rate, n_epochs=n_epochs, **kwargs) # AO: 'hist' was temporarily 'history'
    ...
    def train_one_batch(self,
        ...
        # Presample data # AO TODO: Experimental feature! Must get rid of!
        ...
        for w_ind in range(self.n_walkers):
            ...
            l_l_data_points = Parallel(n_jobs=num_cores, backend='threading')(delayed(process_one_map)(ind_map,t_map, m_meta_map, t_ch_power, t_sampled_map, m_mask)
            ...
            print('The elapsed time for generating and sampling training and validation maps using multiple processors '
                  'with %d cores is' % num_cores, time.strftime("%H hours %M min %S sec", time.gmtime(elapsed_time))) #time.strftime("%d:%H:%M:%S", time.gmtime(elapsed_time)))
