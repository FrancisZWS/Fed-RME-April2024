...
class MapSampler:
    ...
    def sample_map_customer(self, t_map_to_sample, m_meta_map):
        """
        Returns:
        `t_sampled_map`: Nx x Ny x Nf tensor
        `m_mask`: Nx x Ny, each entry is 1 if that grid point is sampled; 0 otherwise, same mask is applied along Nf dimension.
        """
        sampling_factor = self.v_sampling_factor

        shape_in = t_map_to_sample.shape
        m_mask = np.zeros((shape_in[0], shape_in[1]))  # initialize m_mask
        sampled_map = np.zeros_like(t_map_to_sample)  # initialize sampled_map

        # calculate number of total sample
        total_points_to_sample = int(sampling_factor * shape_in[0] * shape_in[1])

        sampled_points = 0  # number of samples taken

        # index
        row_indices, col_indices = np.where(m_meta_map == 0)  # 0=not building
        coordinates = list(zip(row_indices, col_indices))
        remaining_coordinates = list(coordinates)

        while sampled_points <= total_points_to_sample and remaining_coordinates:
            # random choose one point
            random_index = random.choice(remaining_coordinates)
            remaining_coordinates.remove(random_index)
            # x, y = divmod(random_index, shape_in[1])
            x, y = random_index


            m_mask[x, y] = 1
            sampled_map[x, y, :] = t_map_to_sample[x, y, :]
            sampled_points += 1
        
        # # add noize
        # t_sampled_map_ret = sampled_map + np.multiply(
        #     np.random.normal(loc=0, scale=self.std_noise, size=t_map_to_sample.shape),
        #     np.expand_dims(m_mask, axis=2)
        # )

        return sampled_map, m_mask
